...
MyStlMesh::MyStlMesh(std::string path) {
    std::vector< std::vector<float> > retour = MyStlReader::readStlTextFile(path);
    ...
    {
        int ii[3] = {-1,-1,-1};
        for(int j=0; j<retour[i].size(); j+=3)
        {
            HACD::Vec3<HACD::Real> vertex(retour[i][j], retour[i][j+1],retour[i][j+2]);
