...
QMainWindow(parent), ui(new Ui::MainWindow)
{
  ...
  connect(ui->actionRobot_new_2,SIGNAL(triggered()),this,SLOT(openDirForRobot()));
    connect(ui->button_robot2,SIGNAL(clicked()),ui->actionRobot_new_2,SIGNAL(triggered()));

  connect(ui->actionRobot_new_3,SIGNAL(triggered()),this,SLOT(openDirForRobot()));
    connect(ui->button_robot3,SIGNAL(clicked()),ui->actionRobot_new_3,SIGNAL(triggered()));

  connect(ui->actionRobot_new_4,SIGNAL(triggered()),this,SLOT(openDirForRobot()));
 ...
 */
  ...
  void MainWindow::openDirForRobot(){
    ...
    if(fileName!=NULL){
          QString name=("robot number" +robotNumber); // todo : ask user and signal mapping
