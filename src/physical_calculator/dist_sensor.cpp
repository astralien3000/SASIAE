...
void DistSensor::init(const PositionData pos_data, const PositionData direction, const QVector3D& boxSize, float mass){
 ...
 //lx = -z y x
 btVector3 normal(direction.x,
              direction.y,
              direction.z);
 btVector3 pos(pos_data.x,
              pos_data.y,
              pos_data.z);
 normal.normalize();
 ...
 btTypedConstraint* p2pL = new btPoint2PointConstraint(*((btRigidBody*)*_chassis),*((btRigidBody*)*_sensor_box), pivotInChassis1,pivotInBox1);
 btTypedConstraint* p2pR = new btPoint2PointConstraint(*((btRigidBody*)*_chassis),*((btRigidBody*)*_sensor_box), pivotInChassis2,pivotInBox2);
 
 // add constraint to world
 ((btDiscreteDynamicsWorld*) _world)->addConstraint(p2pL);
 ((btDiscreteDynamicsWorld*) _world)->addConstraint(p2pR);
...
void DistSensor::calcDist(){
  btTransform trans;
 ((btRigidBody*)_sensor_box)->getMotionState()->getWorldTransform(trans);//TODO check if it's the right vect 
btVector3 worldDir = btVector3(0,0,1).rotate(trans.getRotation().getAxis(), trans.getRotation().getAngle()).normalize();
 ...
 ((btDiscreteDynamicsWorld*) _world)->rayTest(start,end,rayCallback);
